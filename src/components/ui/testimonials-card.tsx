    ...
    return (
        <div className={cn("flex items-center justify-center p-8", className)}>
            ...
            >
                {/* Counter */}
                {showCounter && (
                    <div className="row-start-1 md:col-start-2 md:row-start-1 text-right text-sm tabular-nums text-ink-faint">
                    ...
                    <AnimatePresence custom={direction}>
                        {items.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className="absolute inset-0 w-full h-full overflow-hidden border-[6px] bg-pearl-deep border-white shadow-[0_30px_60px_-25px_rgba(22,23,27,0.45)] rounded-2xl"
                                    initial={{
                                        x: offset * 15,
                                        y: Math.abs(offset) * 6,
                ...
                {/* Text Area */}
                <div className="col-start-1 md:col-start-2   md:row-start-1 flex flex-col justify-center min-h-[220px]">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <p className="display text-[clamp(1.2rem,1.9vw,1.6rem)] leading-snug text-ink">
                ...
                {showNavigation && items.length > 1 && (
                    <div className="col-start-1 md:col-start-2 md:row-start-3 flex gap-2  m-auto -mt-2 md:mt-4  md:m-0">
                        <button
                            ...
                            onClick={handlePrev}
                            className={cn(
                                "flex items-center justify-center w-11 h-11 rounded-full border border-line bg-white transition-[background-color,transform] duration-150 active:scale-[0.97]",
                                activeIndex === 0
